...
class Bot:
    ...
    def __init__(self):
        ...
        if Path.exists(self.model_path):
            ...
            self.text_gen_pipe = pipeline('text-generation', model='AlekseyKorshuk/gpt2-jokes', device=self.device, return_full_text=False)
            self.text_gen_pipe.model.config.pad_token_id = self.text_gen_pipe.model.config.eos_token_id

        # Init model to rate jokes
        self.rate_pipe = pipeline(model='Reggie/muppet-roberta-base-joke_detector', device=self.device, truncation=True, max_length=510)
